...
python train_gpus.py --gpuid ${GPU_ID} --batch_size ${Batch_size} --data_path ${DATA_PATH} --data_name ${data_name} --vocab_path vocab --logger_name ${logger_name} --model_name ${model_name} --max_violation --bi_gru --agg_func=Mean --cross_attn=t2i --lambda_softmax=${lambda_softmax} --num_epochs=20 --lr_update=10 --learning_rate=.0005 --iteration_step ${iteration_step} --model_mode ${model_mode} --no_IMRAM_norm
